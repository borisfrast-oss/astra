...
| `multi_group` | typing.Optional[astro_process.config.models.MultiGroupConfig] |  | - |
| `registration` | typing.Optional[astro_process.config.models.RegistrationConfig] |  | - |
| `gradient_removal` | typing.Optional[astro_process.config.models.GradientRemovalConfig] |  | - |
...
| `rejection_thresholds` | typing.Optional[dict[str, tuple[typing.Optional[float], typing.Optional[float]]]] |  | - |
| `rejection_elongation` | typing.Optional[bool] |  | - |
| `rejection_min_corr_hp` | typing.Optional[float] | 0.05 | - |
| `frame_selection` | typing.Optional[astro_process.config.models.FrameSelectionConfig] |  | - |
| `cosmetic_correction` | typing.Optional[astro_process.config.models.CosmeticCorrectionConfig] |  | - |
...
| `pcc` | <class 'astro_process.config.models.PCCConfig'> | PydanticUndefined | - |
| `suggest` | <class 'astro_process.config.models.SuggestConfig'> | PydanticUndefined | - |
| `filename_patterns` | typing.Optional[astro_process.config.models.FilenamePatterns] |  | - |
| `cfa_drizzle` | typing.Optional[astro_process.config.models.CFADrizzleConfig] |  | - |
| `debayer_method` | typing.Optional[typing.Literal['superpixel', 'bilinear', 'malvar']] |  | Default debayer algorithm: superpixel, bilinear, or malvar. |
| `preview` | <class 'astro_process.config.models.PreviewConfig'> | PydanticUndefined | - |
| `export` | typing.Optional[astro_process.config.models.ExportConfig] |  | - |
...
| `equipment_profiles` | list[astro_process.config.models.EquipmentProfile] | PydanticUndefined | - |
| `pipeline_presets` | list[astro_process.config.models.PipelinePreset] | PydanticUndefined | - |
...
| `vizier_apass_timeout` | <class 'float'> | 30.0 | Timeout in seconds for APASS catalog queries. |
| `vizier_refcat2_timeout` | <class 'float'> | 30.0 | Timeout in seconds for REFCAT2 catalog queries. |
| `registration` | <class 'astro_process.config.models.RegistrationConfig'> | PydanticUndefined | - |
| `gradient_removal` | <class 'astro_process.config.models.GradientRemovalConfig'> | PydanticUndefined | - |
| `debayer_method` | typing.Literal['superpixel', 'bilinear', 'malvar'] | superpixel | Debayer algorithm: superpixel, bilinear, or malvar. |
...
| `rejection_thresholds` | dict[str, tuple[typing.Optional[float], typing.Optional[float]]] | PydanticUndefined | Per-metric lower/upper thresholds for rejection. |
| `rejection_elongation` | <class 'bool'> | True | Reject frames based on elongation metrics. |
| `rejection_min_corr_hp` | typing.Optional[float] | 0.05 | - |
| `frame_selection` | <class 'astro_process.config.models.FrameSelectionConfig'> | PydanticUndefined | - |
| `preview_export` | <class 'astro_process.config.models.PreviewExportConfig'> | PydanticUndefined | - |
...
| `pcc_fallback` | typing.Literal['gray_world', 'skip', 'fail', 'auto'] | auto | Fallback strategy when PCC fails for a group. |
| `pcc_per_group` | <class 'bool'> | False | Apply PCC to each group stack before merging. |
| `merge` | <class 'astro_process.config.models.MergeConfig'> | PydanticUndefined | Merge strategy configuration for combining group stacks. |
...
| Field | Type | Default | Description |
| --- | --- | --- | --- |
| `method` | typing.Literal['weighted_average', 'average', 'median'] | weighted_average | Pixel-combining method for merging group stacks. |
| `weight_by` | typing.Literal['frame_count', 'total_exposure'] | frame_count | Weighting criterion for merging groups. |
...
| Field | Type | Default | Description |
| --- | --- | --- | --- |
| `method` | typing.Literal['fft', 'astroalign', 'rotation_fft'] | fft | Registration engine: astroalign, fft, or rotation_fft. |
...
| `max_exptime_fft_warn` | <class 'float'> | 45.0 | Threshold in seconds above which a warning is emitted for fft registration on AZ mounts. |
...
| `kernel` | typing.Literal['lanczos3', 'gaussian', 'tophat'] | lanczos3 | Drizzle kernel: lanczos3, gaussian, or tophat. |
| `quality_gate` | <class 'astro_process.config.models.CFADrizzleQualityGateConfig'> | PydanticUndefined | Quality thresholds that must pass before drizzle is applied. |
| `min_frames` | <class 'int'> | 5 | Minimum frame count required to enable drizzle. |
| `fallback` | typing.Literal['malvar', 'superpixel', 'skip'] | malvar | Fallback debayer method if drizzle preconditions fail. |
...
### 3. .env File (optional, cross-platform)

```dotenv
# .env — optional, for convenience (do not commit secrets)
...
astra init --non-interactive
astra doctor          # checks config, warns WARN doctor.env_missing if .env absent, but pipeline uses defaults
```

`astra doctor` (read-only) checks: Python, dependencies, GAIA, `config.yaml`, disk, paths, and `.env` existence. Precedence for config discovery: explicit `--config path` > `CWD/config.yaml` > `pipeline_root/config.yaml` > `DEFAULT_CONFIG` string (wheel fallback). Each discovery is logged as `config.loaded_from` (source, path).
