"""Tests for the dark filename pattern (dark_dwarf3) in fits_parser.
...
class TestCompactDarkPattern:
    ...
    das Muster NACH dark/dark_dwarf3-Pattern (die liefern mehr Metadaten), mit
    ...
    def test_dark_dwarf3_pattern_unchanged(self):
        """Gegenprobe: dark_dwarf3-Pattern gewinnt weiterhin (temp/stack nicht verloren)."""
        meta = parse_filename_metadata(Path("dark_15s_60g_30C_1.fits"))
        assert meta["exptime"] == "15"
        assert meta["gain"] == "60"
        assert meta["temp"] == "30"
